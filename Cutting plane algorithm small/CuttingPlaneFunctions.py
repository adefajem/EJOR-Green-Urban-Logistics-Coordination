...
def run_instance(city_name, problem_instance):
    ...
    time_limit_per_iteration = 600
    problem_time_limit = 3600
#     try:
    timer = pc()
    bilevel_HPR_emissions_model = HPR_model_emissions(time_limit_per_iteration, package_ids, destinations, selected_locker_nodes, max_locker_capacities, num_FirstMilers, num_DSPs, num_vehicles_per_FM, num_vehicles_per_DSP, all_nodes_first_echelon,
    ...
    # Follower & interdiction cuts #  cutting_plane_algorithm_FOLLOWER_AND_INTERDICTION
    hpr_sol_final, lamda_sol_final, w_sol_final, xm_sol_final, y_sol_final, first_m_sol_dfs_final, last_m_sol_dfs_final, first_m_sol_dfs_FIRSTITER, last_m_sol_dfs_FIRSTITER, all_iter_results = cutting_plane_algorithm_MODIFIED(bilevel_HPR_emissions_model, Pf, package_ids, destinations, selected_locker_nodes, max_num_nodes, 
    num_FirstMilers, num_DSPs, num_vehicles_per_FM, num_vehicles_per_DSP, cost_per_km_for_FM, cost_per_km_for_DSP,cost_per_DSP_vehicle, emissions_matrix_ICE, emissions_matrix_EV, 
                                ...
                                time_violation_penalty, False, problem_time_limit)
...
def solve_param_last_mile_followers(y_sol, lamda_sol, num_DSPs, num_vehicles_per_DSP, cost_per_km_for_DSP, dsp_depots, dsp_d_nodes, dsp_d_arcs, packages, destinations, locker_nodes, distance_matrix, travel_time_matrix, earliest, latest, bigM_matrix, time_violation_penalty):
    ...
    for d in range(num_DSPs):      
        obj, sol_df = last_mile_follower_static_single(y_sol[:, d], lamda_sol, dsp_d_nodes[d], dsp_d_arcs[d], dsp_depots[d], cost_per_km_for_DSP[d], packages, destinations, locker_nodes, distance_matrix, travel_time_matrix, earliest, latest, bigM_matrix, time_violation_penalty)
