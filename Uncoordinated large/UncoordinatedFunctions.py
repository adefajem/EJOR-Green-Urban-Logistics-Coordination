...
def run_instance(city_name, problem_instance):
    ...
    max_num_FMs = len(city_instance_df.loc[city_instance_df['type'].str.contains('FM')])
    max_num_DSPs = len(city_instance_df.loc[city_instance_df['type'].str.contains('LM')])
    max_num_lockers = len(city_instance_df.loc[city_instance_df['type'].str.contains('locker')])
    max_num_packages = len(city_instance_df.loc[city_instance_df['type'].str.contains('package')])    
    max_num_nodes = len(city_instance_df)
    ...
    max_locker_capacities[8] = 40
    max_locker_capacities[9] = 50
    ...
    each_dsp_cap = np.ceil((1.2*len(package_ids)/current_num_DSPs)) #+ (len(package_ids)/10)
    dsp_capacities = []
...
def solve_param_last_mile_followers_static_single(y_sol_cl, lamda_sol_cl, dsp_d_nodes, dsp_d_arcs, dsp_depots,
    ...
    for d in range(len(dsp_depots)):  
        ...
        obj_val, sol_df = last_mile_follower_static_single_cp(y_sol_cl[:, d], lamda_sol_cl, dsp_d_nodes[d], dsp_d_arcs[d], dsp_depots[d],
...
def last_mile_follower_static_single_cp(y, lamda, V2d, A2d, fol_depot, cost_per_km, packages, destinations, locker_nodes, distance_matrix, travel_time_matrix, earliest, latest, bigM_matrix, time_violation_penalty):
    ...
    model.parameters.emphasis.mip = 0 # default: balance optimality and feasibility
